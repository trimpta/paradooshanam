...
# Replace the entire `{mainTab === 'IN' ? ( ... ) : ( ... )}` with cleanly rendered layout
in_tab_regex = r"\{mainTab === 'IN' \? \(\s*<>\s*\{\/\* Top: Current Record Input \*\/\}.*?\{\/\* Bottom: Header\/Menu OR Score Selector \*\/\}"
...
content = re.sub(in_tab_regex, new_in_tab, content, flags=re.DOTALL)
